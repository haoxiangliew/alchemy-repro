...
PATCH_PKG="@distilled.cloud/cloudflare-runtime@0.10.2"
PATCH_PATH="patches/@distilled.cloud%2Fcloudflare-runtime@0.10.2.patch"
RUNTIME_PKG="@distilled.cloud/cloudflare-runtime"
...
interrupted() {
    echo
    echo ">>> interrupted — tearing down workerd/alchemy and restoring package.json"
...
workerd_mjs() {
    find node_modules/.bun -path "*/@distilled.cloud/cloudflare-runtime/dist/node/workerd/Workerd.mjs" 2>/dev/null | head -1
}

verify_runtime() {
    local f ver
    f=$(workerd_mjs)
    ver=$(echo "$f" | sed -n 's/.*cloudflare-runtime@\([0-9.]*\).*/\1/p')
    if [ -n "$f" ] && grep -q "allocatePort" "$f" 2>/dev/null; then
        echo "PATCHED @${ver:-?} (port pre-alloc + TCP readiness poll, no --control-fd)"
    elif [ -n "$f" ] && grep -q "control-fd=3" "$f" 2>/dev/null; then
        echo "UNPATCHED @${ver:-?} (--control-fd=3, fd-3 port report)"
    else
        echo "UNKNOWN (no $RUNTIME_PKG Workerd.mjs found)"
    fi
}
...
install_suite() { # patched|unpatched
    ...
    bun install >"/tmp/repro-install-$1.log" 2>&1 ||
        {
            echo "    !! bun install failed — see /tmp/repro-install-$1.log"
            tail -5 "/tmp/repro-install-$1.log"
        }
...
run_once() { # logfile
    ...
    while kill -0 "$RUN_PID" 2>/dev/null; do
        started_count=$(grep -c "Started in" "$log" 2>/dev/null)
    ...
    RUN_PID=""
    local final
    final=$(grep -c "Started in" "$log" 2>/dev/null)
    final=${final:-0}
...
case "$MODE" in
unpatched)
    install_suite unpatched
    run_suite unpatched
    ;;
patched)
    install_suite patched
    run_suite patched
    ;;
both)
    install_suite unpatched
